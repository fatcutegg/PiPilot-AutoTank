...
GitHub を使うには、まずアカウント（会員登録）が必要です。まだ持っていない人は、以下のステップでアカウントを作りましょう！

1. ブラウザで [GitHub 公式サイト](https://github.com/) にアクセスします。
2. 右上の **「Sign up」**（新規登録）ボタンをクリックします。
3. メールアドレスを入力し、パスワードとユーザー名（英数字）を設定します。
   > **⚠️ 注意（中学生向け）：**
   > ここで登録したメールアドレスは、後の SSH Key 設定（STEP 1）でも使用するので、忘れないようにメモしておいてくださいね！
   > ユーザー名はインターネット上に公開されるので、本名ではなくニックネームがおすすめです。
4. 画面の指示に従って簡単な人間証明パズルを解き、登録したメールアドレスに届く認証コードを入力します。
5. アカウント作成が完了したら、ログインした状態にしておきましょう！
